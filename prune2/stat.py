...
def main():
    ...
    for i in range(10):
        ...
        num_candidates_class_i =  sum(len(layer) for layer in candidates)
        total_pruned += num_candidates_class_i
        class_i_candidates_flat = []
        print("Num candidate for class {}: ".format(i), num_candidates_class_i)
    ...
    avg_num_prune = total_pruned / 10.0
    # print(global_shared)
    ...
    shared_matrix = [[[] for _ in range(10)] for _ in range(10)]  # Each entry in the shared matrix contains the channels class i shares with class j
...
def rank_by_least_coupled(shared_matrix, i):
    scores = [(k, num_coupled(shared_matrix, i, k)) for k in range(10) if k != i]
