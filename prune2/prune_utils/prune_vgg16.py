...
def prune_vgg16_conv_layer(model, layer_index, filter_index, use_batch_norm=False):
    ...
    if not next_conv is None:
        ...
        if next_conv.bias is not None:
            next_new_conv.bias.data = torch.from_numpy(
                next_conv.bias.data.cpu().numpy().copy()).cuda()
    ...
    if not next_conv is None:
        ...
        new_linear_layer.bias.data = torch.from_numpy(
            old_linear_layer.bias.data.cpu().numpy()).cuda()
...
def prune_last_fc_layer(model, class_indices, use_bce=False):
    ...
    new_linear_layer.bias.data[bce_offset:] = torch.from_numpy(
        np.asarray(old_linear_layer.bias.data.cpu().numpy()[class_indices])).cuda()
